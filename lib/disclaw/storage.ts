...
function mapGuildRow(row: GuildSettingsRow): GuildConfig {
  ...
  const rawMention = row.alert_mention;
  const alertMention: GuildConfig["alertMention"] =
    rawMention === "everyone" || rawMention === "here" || rawMention === "none"
      ? rawMention
      : undefined;
...
class SupabaseStorage implements Storage {
  ...
  async saveGuildConfig(config: GuildConfig): Promise<void> {
    ...
    const payload = {
      ...
      alert_mention: config.alertMention ?? null,
